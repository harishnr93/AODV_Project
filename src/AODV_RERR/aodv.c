...
{
    // Creating timer and delay objects  
	...
	// Assigning destination with maximum node value
        dst = NET_NODES;
...
PROCESS_THREAD(rreq_pck, ev, data)
{
	...
	//Entry made in discovery table
        ...
        fwdrreq(&rreq);    //Broadcast the ROUTE_REQUEST
    }
...
static void unicast_rrep(struct unicast_conn *c, const linkaddr_t *from)
{
    ...
    if(((*rrep).linkage) == 0)
	{
			printf("Route Error at  Source : %d and Destination : %d \n", (*rrep).src,(*rrep).dst);
...
static void broadcast_rreq(struct broadcast_conn *c, const linkaddr_t *from)
{
    ...
    printf("# RREQ received from %d \n\n# | Entry : %d | Node : %d | Dest : %d |\n",
	    ...
	    rrep.req_ent= (*rreq).req_ent;
            ...
            rrep.dst = (*rreq).dst;
            rrep.hops = 0;
	    rrep.linkage = 0; // linkage

        //ROUTE_REPLY to the source of ROUTE_REQ 
